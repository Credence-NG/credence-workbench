...
const Index = () => {
	...
	const createClientCredentials = async () => {
		...
		if (orgId) {
			setLoading(true);
			try {
				console.log('Creating credentials for orgId:', orgId);
				const response = await createCredentials();
				const { data } = response as AxiosResponse;
				console.log('Create credentials response:', data);
					...
					<ul className="flex flex-wrap -mb-px text-sm font-medium text-center">
						<li className="mr-2">
							<button
								...
								className={`inline-block p-4 border-b-2 rounded-t-lg ${activeTab === 'credentials'
						...
						<li className="mr-2">
							<button
								...
								className={`inline-block p-4 border-b-2 rounded-t-lg ${activeTab === 'webhooks'
									? 'text-primary-600 border-primary-600 dark:text-primary-500 dark:border-primary-500'
									: 'border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300'
									}`}
							>
