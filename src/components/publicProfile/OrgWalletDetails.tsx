...
const OrgWalletDetails = ({ orgData }: IExploreOrg) => {
	...
	useEffect(() => {
		if (orgData && orgData?.org_agents && typeof orgData?.org_agents === 'object') {
					...
					<div className="max-w-7xl">
						...
						<div className={`flex justify-between flex-wrap`}>
							<div className="mb-4 sm:mb-0 px-0 sm:px-4 py-4 min-w-full lg:min-w-[550px]" style={{ width: "calc(100% - 23rem)" }}>
								{orgData?.org_agents && typeof orgData?.org_agents === 'object' &&
									<div className="">
											...
											{orgData?.org_agents ?
												<>
													<li className="flex z-10 items-center text-xl">
														...
														<CopyDid className="ml-4 lg:ml-9 text-gray-600 dark:text-gray-400 truncate font-courier pt-2" value={orgData?.org_agents?.orgDid} />
													</li>
													<li className="flex items-center text-xl">
														<span className="shrink-0 w-fit lg:w-40">Ledger</span>
														<span className="mx-2">:</span>
														<span className="ml-4 lg:ml-9 text-gray-600 dark:text-gray-400">{orgData?.org_agents?.ledgers?.name}</span>
													</li>
													<li className="flex items-center text-xl">
														<span className="shrink-0 w-fit lg:w-40">Network</span>
														<span className="mx-2">:</span>
														<span className="ml-4 lg:ml-9 text-gray-600 dark:text-gray-400">{orgData?.org_agents?.ledgers?.networkType}</span>
													</li>
												</>
												:
												<ul>
