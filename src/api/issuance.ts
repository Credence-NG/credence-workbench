...
export const resendCredential = async (credentialRequestId: string) => {
  const orgId = await getFromLocalStorage(storageKeys.ORG_ID);
  const url = `${apiRoutes.organizations.root}/${orgId}${apiRoutes.Issuance.resendCredential}/${credentialRequestId}`;
